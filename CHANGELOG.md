...
**A grant is redeemed for the tree too.** The task message's `grant` said it was redeemed for input URLs, secret values and upload URLs, and left out the workflow tree the redemption's response already requires. The engine now serves the tree that way, one presigned GET per file, so the first description a runner author reads says so.
