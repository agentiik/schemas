...
| --- | --- | --- |
| [`workflow.schema.json`](workflow.schema.json) | `https://schemas.agentiik.dev/workflow.schema.json` | `agentiik.yaml`, the entry point of a workflow repository: `apiVersion`, `kind`, `metadata`, `inputs`, `outputs`, `on`, `mcp`, `include`, `vars`, `secrets`, `defaults`, `concurrency` and `steps`, with every step keyword. |
...
| --- | --- | --- |
| `grace` | The step keyword table names it on the `timeout` row, "SIGKILL after grace", and never says whether it is a step keyword, a sub-key of `timeout` or a runner policy. | No keyword is declared. Choosing one of the three would be inventing product behaviour. Recorded as a `$comment` on `workflow.$defs/timeout`. |
...
| An exit code in a manifest | The exit code table says a manifest declaring a reserved code is refused, and no manifest property carries a code. | No definition is published. The range is 0 success, 1 to 99 application failure, 100 to 119 transient, 120 invalid input, 121 to 124 reserved and refused at manifest validation, 125 and above reserved for the runtime; it is kept here rather than as a `$defs` entry nothing refers to, because the language reference is a projection of these documents and would otherwise teach a manifest key that does not exist. |
