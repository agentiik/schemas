...
| [`wire.schema.json`](wire.schema.json) | `https://schemas.agentiik.dev/wire.schema.json` | Every message between the controller, the bus, the runner and the API: the task message and the task result, a runner's registration, its heartbeat, the redemption of a task's grant, a log shipment, and a runner pool with the token issued from it. |
